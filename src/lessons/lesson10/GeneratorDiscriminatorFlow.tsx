...
export default function GeneratorDiscriminatorFlow() {
  return (
    <div className="rounded-2xl border border-slate-200 bg-slate-50 p-5" role="img" aria-label="생성 입력이 생성자에 들어가 생성 데이터를 만들고, 실제 데이터와 생성 데이터가 감별자에 들어가 실제인지 생성인지 판단하며 피드백이 생성자의 다음 개선에 이어지는 흐름">
      <div className="flex flex-col items-stretch gap-3 md:flex-row md:items-center">
        <div className="rounded-xl border border-slate-300 bg-white p-4 text-center md:flex-1"><strong>생성 입력</strong><span className="mt-1 block text-sm">새 결과를 만들기 위한 시작 정보<br />실제 데이터를 그대로 복사한 값이 아님</span></div>
        <Arrow />
        ...
        <Arrow />
        <div className="rounded-xl border border-emerald-300 bg-emerald-50 p-4 text-center md:flex-1"><strong>판단과 피드백</strong><span className="mt-1 block text-sm">생성자가 다음에 개선할 방향을 얻음</span></div>
      </div>
    </div>
  )
}
