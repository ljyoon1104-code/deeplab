...
export function Lesson12Step1(props: CommonStepProps) {
  ...
  const ready = allCorrect && activity.sequenceConfirmed
  return <StepFrame {...props} step={1} intro="비슷한 단어라도 나타나는 순서와 관계가 달라지면 누가 무엇을 했는지가 바뀝니다.">
    ...
    {allCorrect ? <label className="mt-4 flex items-start gap-3 rounded-xl bg-violet-50 p-4"><input type="checkbox" className="mt-1 size-5" checked={activity.sequenceConfirmed} onChange={(event) => update((current) => ({ ...current, sequenceConfirmed: event.target.checked }))} /><span>두 문장 쌍에서 단어 순서가 행동의 주체와 대상을 바꾸어 문장 의미에 영향을 준다는 점을 확인했습니다.</span></label> : null}
...
export function Lesson12Step2(props: CommonStepProps) {
  ...
  const ready = allViewed && activity.rnnFlowConfirmed && activity.rnnConceptConfirmed
...
export function Lesson12Step3(props: CommonStepProps) {
  ...
  const ready = allViewed && activity.longContextCorrect && activity.rnnLimitConfirmed
  ...
  return <StepFrame {...props} step={3} intro="앞부분의 정보와 마지막 행동 사이가 가까운 문장과 먼 문장을 비교합니다.">
    ...
    {allViewed && activity.longContextCorrect ? <label className="mt-5 flex items-start gap-3 rounded-xl bg-cyan-50 p-4"><input type="checkbox" className="mt-1 size-5" checked={activity.rnnLimitConfirmed} onChange={(event) => update((current) => ({ ...current, rnnLimitConfirmed: event.target.checked }))} /><span>기본 RNN의 긴 문맥 한계는 가능성에 대한 설명이며, 모든 RNN이 동일하거나 긴 문장을 전혀 처리하지 못한다는 단정이 아님을 확인했습니다.</span></label> : null}
...
export function Lesson12Step4(props: CommonStepProps) {
  const { activity, update } = useLesson12()
  const correct = activity.attentionWords.length === 2 && activity.attentionWords.includes('비') && activity.attentionWords.includes('우산')
  const ready = correct && activity.attentionContextConfirmed && activity.attentionSimulationConfirmed
  const toggle = (word: string) => update((current) => { const selected = current.attentionWords.includes(word) ? current.attentionWords.filter((item) => item !== word) : current.attentionWords.length < 2 ? [...current.attentionWords, word] : [current.attentionWords[1], word]; return { ...current, attentionWords: selected, attentionContextConfirmed: false } })
  return <StepFrame {...props} step={4} intro="문장 속 단어를 없애지 않고, 현재 질문에 중요한 관계를 더 크게 반영하는 아이디어를 살펴봅니다.">
    <div className="rounded-2xl border border-slate-200 bg-slate-50 p-5"><p className="text-sm font-black text-slate-600">예문</p><p className="mt-2 text-xl font-black">민수는 비가 많이 와서 우산을 챙겼다.</p><p className="mt-3 font-bold">민수가 우산을 챙긴 이유를 이해하는 데 중심이 되는 단어 관계는 무엇일까요?</p></div>
    <div className="mt-5"><AttentionVisualizer selected={activity.attentionWords} onToggle={toggle} /></div>
    {activity.attentionWords.length === 2 ? <Feedback correct={correct}>{correct ? <><strong>맞았습니다.</strong> ‘비가 많이 와서’와 ‘우산을 챙겼다’의 관계가 이유를 이해하는 중심 단서입니다.</> : <><strong>다시 살펴보세요.</strong> 우산을 챙긴 직접적인 이유와 연결되는 두 단어를 선택하세요.</>}</Feedback> : null}
    <Notice><strong>Attention</strong>은 문장 안에서 현재 처리에 중요한 단어들의 관계를 더 크게 반영하는 아이디어입니다. 중요하지 않다고 판단한 단어를 모두 삭제하는 기능은 아닙니다.</Notice>
    {correct ? <div className="mt-5 grid gap-3"><label className="flex items-start gap-3 rounded-xl bg-amber-50 p-4"><input type="checkbox" className="mt-1 size-5" checked={activity.attentionContextConfirmed} onChange={(event) => update((current) => ({ ...current, attentionContextConfirmed: event.target.checked }))} /><span>굵은 주황 연결선은 ‘비 ↔ 우산’의 중요 관계를 나타내고, 다른 단어도 문장에서 삭제되지 않았음을 확인했습니다.</span></label><label className="flex items-start gap-3 rounded-xl bg-slate-50 p-4"><input type="checkbox" className="mt-1 size-5" checked={activity.attentionSimulationConfirmed} onChange={(event) => update((current) => ({ ...current, attentionSimulationConfirmed: event.target.checked }))} /><span>이 화면은 실제 Attention 점수 계산 결과가 아닌 교육용 개념 시뮬레이션임을 확인했습니다.</span></label></div> : null}
    <Button className="mt-5" disabled={!ready} onClick={props.onComplete}>STEP 4 활동 완료</Button>{props.isComplete ? <Completion>‘비’와 ‘우산’의 중요 관계와 Attention의 교육용 범위를 확인했습니다.</Completion> : null}
  </StepFrame>
}
...
export function Lesson12Step6(props: CommonStepProps) {
  ...
  const nextComplete = activity.nextFirstChoice === '우유를' && Boolean(activity.nextSecondChoice) && activity.nextWordSelections.length >= 2
  const ready = frequencyCorrect && activity.probabilitiesConfirmed && nextComplete && activity.generationSimulationConfirmed
  return <StepFrame {...props} step={6} intro="작은 고정 빈도표로 후보 확률을 계산하고, 선택한 다음 단어가 새 문맥에 더해지는 과정을 관찰합니다.">
    ...
    <div className="mt-6"><FrequencyProbabilityTable answers={activity.frequencyAnswers} submitted={activity.frequencySubmitted} onChoose={(context, word) => update((current) => ({ ...current, frequencyAnswers: { ...current.frequencyAnswers, [context]: word } }))} onSubmit={(context) => update((current) => ({ ...current, frequencySubmitted: [...new Set([...current.frequencySubmitted, context])] }))} onRetry={(context) => update((current) => ({ ...current, frequencySubmitted: current.frequencySubmitted.filter((item) => item !== context), frequencyAnswers: { ...current.frequencyAnswers, [context]: '' } }))} /></div>
    {frequencyCorrect ? <label className="mt-5 flex items-start gap-3 rounded-xl bg-cyan-50 p-4"><input type="checkbox" className="mt-1 size-5" checked={activity.probabilitiesConfirmed} onChange={(event) => update((current) => ({ ...current, probabilitiesConfirmed: event.target.checked }))} /><span>실제 고정 빈도에서 계산된 확률 막대가 밥: 먹다 97.2%·타다 0.0%·마시다 2.8%, 버스: 0.0%·100.0%·0.0%, 커피: 11.0%·1.1%·87.9%로 나타나는 것을 확인했습니다.</span></label> : null}
    <div className="mt-9 border-t border-slate-200 pt-7"><h3 className="text-xl font-black">확률에 따라 문장 이어 보기</h3><p className="mt-2 text-sm leading-6 text-slate-600">후보를 누르면 선택한 단어가 문맥에 추가됩니다. 기본 학습 경로인 ‘우유를’을 선택하면 두 번째 후보가 나타납니다.</p><div className="mt-5"><NextWordSimulation firstChoice={activity.nextFirstChoice} secondChoice={activity.nextSecondChoice} onFirstChoice={(word) => update((current) => ({ ...current, nextFirstChoice: word, nextSecondChoice: word === current.nextFirstChoice ? current.nextSecondChoice : '', nextWordSelections: [...current.nextWordSelections, word] }))} onSecondChoice={(word) => update((current) => ({ ...current, nextSecondChoice: word, nextWordSelections: [...current.nextWordSelections, word] }))} /></div></div>
...
export function Lesson12Step7({ priorStepsComplete, onCompletionReadyChange, ...props }: Step7Props) {
  ...
  const allQuizSubmitted = LESSON12_QUIZ.every((item) => activity.quizSubmitted.includes(item.id))
  const ready = activity.languageModelConfirmed && activity.llmConfirmed && allUsesViewed && activity.roadmapConfirmed && allQuizSubmitted
