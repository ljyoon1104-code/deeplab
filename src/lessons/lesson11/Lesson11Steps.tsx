...
import {
  ...
  WORDS,
  lesson11StepTitles,
  type NlpActivityType, TEXT_REPRESENTATION_SCENARIOS,
} from './lesson11Data'
...
import { countWords, createWordIndex, frequencyRatio, tokenizeText, validateOneHotVector } from './textRepresentation'
...
export function Lesson11Step3(props: CommonStepProps) {
  ...
  const [unknownSubmitted, setUnknownSubmitted] = useState(false)
  ...
  const ready = allCorrect && activity.indexReverseCorrect && activity.unregisteredWordCorrect && activity.indexImportanceConfirmed
  return <StepFrame {...props} step={3} intro="신경망이 계산할 수 있도록 문장을 단어로 나누고 각 단어에 구별 번호를 연결합니다.">
    ...
    <div className="mt-6 grid gap-4 md:grid-cols-2">
      <fieldset className="rounded-2xl border border-slate-200 p-5"><legend className="px-2 font-black">인덱스 → 단어 역연결</legend><p className="text-sm leading-6">인덱스 2에 있는 단어는 무엇인가요?</p><ChoiceButtons options={WORDS.map((word) => ({ id: word, text: word }))} value={reverseAnswer} disabled={reverseSubmitted} onChoose={setReverseAnswer} />{!reverseSubmitted ? <Button className="mt-4" disabled={!reverseAnswer} onClick={() => { const correct = reverseAnswer === WORDS[2]; setReverseSubmitted(true); update((current) => ({ ...current, indexReverseCorrect: correct })) }}>역연결 제출</Button> : <Feedback correct={activity.indexReverseCorrect}><strong>{activity.indexReverseCorrect ? '정답입니다.' : '오답입니다.'}</strong> 인덱스 2는 “별처럼”입니다.{!activity.indexReverseCorrect ? <Button className="mt-3" variant="secondary" onClick={() => { setReverseAnswer(''); setReverseSubmitted(false); update((current) => ({ ...current, indexReverseCorrect: false })) }}><RotateCcw size={17} /> 다시 풀기</Button> : null}</Feedback>}</fieldset>
      <fieldset className="rounded-2xl border border-slate-200 p-5"><legend className="px-2 font-black">미등록 단어 판단</legend><p className="text-sm leading-6">목록에 없는 “{UNREGISTERED_WORD}”을 기존 단어의 번호에 임의로 연결해도 될까요?</p><ChoiceButtons options={[{ id: 'no', text: '안 된다. 뜻이 섞일 수 있으므로 별도 처리(<UNK> 등)가 필요할 수 있다.' }, { id: 'yes', text: '된다. 가장 가까운 번호에 무조건 넣으면 된다.' }]} value={unknownAnswer} disabled={unknownSubmitted} onChoose={setUnknownAnswer} />{!unknownSubmitted ? <Button className="mt-4" disabled={!unknownAnswer} onClick={() => { const correct = unknownAnswer === 'no'; setUnknownSubmitted(true); update((current) => ({ ...current, unregisteredWordCorrect: correct })) }}>판단 제출</Button> : <Feedback correct={activity.unregisteredWordCorrect}><strong>{activity.unregisteredWordCorrect ? '정답입니다.' : '오답입니다.'}</strong> 미등록 단어는 기존 단어와 같은 인덱스를 임의로 쓰지 않습니다.{!activity.unregisteredWordCorrect ? <Button className="mt-3" variant="secondary" onClick={() => { setUnknownAnswer(''); setUnknownSubmitted(false); update((current) => ({ ...current, unregisteredWordCorrect: false })) }}><RotateCcw size={17} /> 다시 풀기</Button> : null}</Feedback>}</fieldset>
    </div>
    <Notice><strong>인덱스는 단어를 구별하기 위해 붙인 번호</strong>입니다. ‘해’의 인덱스가 5라고 해서 ‘좋은’의 인덱스 0보다 더 중요하거나 의미가 5배 크다는 뜻은 아닙니다.</Notice>
    {allCorrect && activity.indexReverseCorrect && activity.unregisteredWordCorrect ? <label className="mt-4 flex items-start gap-3 rounded-xl bg-violet-50 p-4"><input type="checkbox" className="mt-1 size-5" checked={activity.indexImportanceConfirmed} onChange={(event) => update((current) => ({ ...current, indexImportanceConfirmed: event.target.checked }))} /><span>인덱스의 크기는 단어의 중요도나 의미의 크기를 나타내지 않는다는 설명을 확인했습니다.</span></label> : null}
...
export function Lesson11Step4(props: CommonStepProps) {
  ...
  const invalidVector = [1, 0, 1, 0, 0, 0]
  const invalidResult = validateOneHotVector(invalidVector, WORDS.length, 0)
  const ready = enoughViewed && activity.oneHotLengthCorrect && activity.oneHotInvalidVectorCorrect && activity.oneHotLimitsConfirmed
  ...
  return <StepFrame {...props} step={4} intro="단어의 인덱스 위치 하나만 1로 바꾸어 길이 6의 원-핫 벡터를 직접 관찰합니다.">
    ...
    <fieldset className="mt-6 rounded-2xl border border-slate-200 p-5"><legend className="px-2 font-black">오류 원-핫 벡터 찾기</legend><p className="font-mono">[{invalidVector.join(', ')}]</p><p className="mt-2 text-sm">이 벡터가 “좋은”(인덱스 0)의 올바른 원-핫 벡터인가요?</p><ChoiceButtons options={[{ id: 'invalid', text: '아니다. 값 1이 두 개라서 올바르지 않다.' }, { id: 'valid', text: '그렇다. 1이 하나 이상이면 된다.' }]} value={activity.oneHotInvalidVectorAnswer} onChoose={(answer) => update((current) => ({ ...current, oneHotInvalidVectorAnswer: answer, oneHotInvalidVectorCorrect: answer === 'invalid' }))} />{activity.oneHotInvalidVectorAnswer ? <Feedback correct={activity.oneHotInvalidVectorCorrect}><strong>{activity.oneHotInvalidVectorCorrect ? '정답입니다.' : '오답입니다.'}</strong> {invalidResult.reason}</Feedback> : null}</fieldset>
    {enoughViewed && activity.oneHotLengthCorrect && activity.oneHotInvalidVectorCorrect ? <label className="mt-5 flex items-start gap-3 rounded-xl bg-amber-50 p-4"><input type="checkbox" className="mt-1 size-5" checked={activity.oneHotLimitsConfirmed} onChange={(event) => update((current) => ({ ...current, oneHotLimitsConfirmed: event.target.checked }))} /><span>원-핫은 한 위치만 1이고 나머지는 0입니다. 단어가 많아지면 벡터가 매우 길고 대부분 0인 <small>희소 표현(Sparse Representation)</small>이 되며, 단어 사이 의미 관계를 직접 표현하기 어렵다는 특징과 한계를 확인했습니다.</span></label> : null}
...
export function Lesson11Step6(props: CommonStepProps) {
  const { activity, update } = useLesson11()
  const frequencyTokens = tokenizeText('축구 축구 경기 선수')
  const frequencyCounts = countWords('축구 축구 경기 선수', ['축구'])
  const expectedCount = frequencyCounts.축구
  const expectedRatio = frequencyRatio(expectedCount, frequencyTokens.length)
  const topicsCorrect = activity.frequencyTopics.doc1 === 'sports' && activity.frequencyTopics.doc2 === 'food'
  const ready = topicsCorrect && activity.frequencyMeaningCorrect && activity.frequencyCountCorrect && activity.frequencyRatioCorrect && activity.frequencyConfirmed
  ...
  return <StepFrame {...props} step={6} intro="단어가 나온 횟수로 문서 주제를 찾고, 단어 순서와 부정 표현을 놓칠 수 있는 한계도 확인합니다.">
    ...
    <fieldset className="mt-6 rounded-2xl border border-slate-200 p-5"><legend className="px-2 font-black">실제 빈도와 비율 계산</legend><p className="text-sm leading-6">문서 1의 실제 토큰은 [{frequencyTokens.join(', ')}]입니다. “축구”의 등장 횟수와 빈도 비율을 계산하세요. 비율은 0.5, 50%, 0,5처럼 입력할 수 있습니다.</p><div className="mt-4 grid gap-3 sm:grid-cols-2"><label className="font-bold">등장 횟수<input value={activity.frequencyCountAnswer} inputMode="numeric" onChange={(event) => update((current) => ({ ...current, frequencyCountAnswer: event.target.value, frequencyCountCorrect: Number(event.target.value.trim()) === expectedCount }))} className="mt-2 min-h-12 w-full rounded-xl border border-slate-300 px-3 font-mono" /></label><label className="font-bold">빈도 비율<input value={activity.frequencyRatioAnswer} inputMode="decimal" onChange={(event) => { const raw = event.target.value.trim().replace(',', '.'); const normalized = raw.endsWith('%') ? Number(raw.slice(0, -1)) / 100 : Number(raw); update((current) => ({ ...current, frequencyRatioAnswer: event.target.value, frequencyRatioCorrect: Number.isFinite(normalized) && Math.abs(normalized - expectedRatio) < 0.005 })) }} className="mt-2 min-h-12 w-full rounded-xl border border-slate-300 px-3 font-mono" /></label></div>{activity.frequencyCountAnswer || activity.frequencyRatioAnswer ? <p className="mt-3 text-sm" aria-live="polite">{activity.frequencyCountCorrect && activity.frequencyRatioCorrect ? '✓ 2회 ÷ 4개 = 0.5 (50%)입니다.' : '횟수는 실제 토큰을 세고, 비율은 횟수 ÷ 전체 토큰 수로 다시 계산해 보세요.'}</p> : null}</fieldset>
    ...
    {topicsCorrect && activity.frequencyMeaningCorrect && activity.frequencyCountCorrect && activity.frequencyRatioCorrect ? <label className="mt-5 flex items-start gap-3 rounded-xl bg-slate-50 p-4"><input type="checkbox" className="mt-1 size-5" checked={activity.frequencyConfirmed} onChange={(event) => update((current) => ({ ...current, frequencyConfirmed: event.target.checked }))} /><span>빈도수 기반 방법은 간단하고 유용하지만, 단어 순서와 세밀한 문맥을 충분히 반영하기 어렵다는 장점과 한계를 확인했습니다.</span></label> : null}
...
export function Lesson11Step7({ priorStepsComplete, onCompletionReadyChange, ...props }: Step7Props) {
  ...
  const quizzesCorrect = LESSON11_QUIZ.every((item) => activity.quizCorrect.includes(item.id))
  const representationsCorrect = TEXT_REPRESENTATION_SCENARIOS.every((item) => activity.representationAssignments[item.id] === item.answer)
  const ready = contextsCorrect && activity.contextOrderNegationCorrect && activity.contextFlowConfirmed && activity.perspectivesConfirmed && representationsCorrect && quizzesCorrect
  ...
  return <StepFrame {...props} step={7} intro="같은 단어라도 주변 단어와 상황에 따라 달라지는 뜻을 찾고 Lesson 11의 전체 흐름을 완성합니다.">
    ...
    <fieldset className="mt-6 rounded-2xl border border-slate-200 p-5"><legend className="px-2 font-black">순서와 부정 표현도 의미를 바꿉니다</legend><div className="grid gap-3 md:grid-cols-2"><p className="rounded-xl bg-slate-50 p-3 font-bold">학생이 선생님을 도왔다.</p><p className="rounded-xl bg-slate-50 p-3 font-bold">선생님이 학생을 도왔다.</p></div><p className="mt-3 text-sm leading-6">두 문장은 같은 두 명과 “도왔다”를 쓰지만, 단어 순서와 관계가 달라 주체와 대상이 바뀝니다. “좋다 / 좋지 않다”처럼 부정어도 전체 의미를 바꿉니다.</p><ChoiceButtons options={[{ id: 'context', text: '단어 순서와 부정 표현을 함께 봐야 의미 차이를 판단할 수 있다.' }, { id: 'frequency', text: '같은 단어가 있으면 빈도만으로 문장 의미가 항상 같다.' }]} value={activity.contextOrderNegationAnswer} onChoose={(answer) => update((current) => ({ ...current, contextOrderNegationAnswer: answer, contextOrderNegationCorrect: answer === 'context' }))} />{activity.contextOrderNegationAnswer ? <Feedback correct={activity.contextOrderNegationCorrect}><strong>{activity.contextOrderNegationCorrect ? '정답입니다.' : '오답입니다.'}</strong> 빈도만으로는 순서, 부정, 누가 누구에게 행동했는지까지 충분히 알기 어렵습니다.</Feedback> : null}</fieldset>
    ...
    {contextsCorrect && activity.contextOrderNegationCorrect ? <label className="mt-5 flex items-start gap-3 rounded-xl bg-cyan-50 p-4"><input type="checkbox" className="mt-1 size-5" checked={activity.contextFlowConfirmed} onChange={(event) => update((current) => ({ ...current, contextFlowConfirmed: event.target.checked }))} /><span>주변 단어와 문장 관계를 확인해야 문맥 속 단어 의미와 문장 전체 의도를 파악할 수 있음을 확인했습니다.</span></label> : null}
    ...
    <div className="mt-8 border-t border-slate-200 pt-7"><h3 className="text-xl font-black">문제에 맞는 텍스트 표현 선택</h3><p className="mt-2 text-sm leading-6 text-slate-600">상황을 읽고 가장 알맞은 표현을 선택하세요. 선택 뒤에는 각 방법의 한계도 함께 확인합니다.</p><div className="mt-5 grid gap-4">{TEXT_REPRESENTATION_SCENARIOS.map((item) => { const selected = activity.representationAssignments[item.id] ?? ''; const correct = selected === item.answer; return <fieldset key={item.id} className="rounded-2xl border border-slate-200 p-5"><legend className="px-2 font-black">{item.text}</legend><div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-5">{([{ id: 'index', text: '단어 index' }, { id: 'onehot', text: '원-핫 벡터' }, { id: 'frequency', text: '단어 빈도수' }, { id: 'embedding', text: '워드 임베딩' }, { id: 'context', text: '문맥 처리 모델' }]).map((option) => <button key={option.id} type="button" aria-pressed={selected === option.id} onClick={() => update((current) => ({ ...current, representationAssignments: { ...current.representationAssignments, [item.id]: option.id } }))} className={`min-h-11 rounded-xl border p-2 text-sm font-bold ${selected === option.id ? 'border-violet-600 bg-violet-50 text-violet-950' : 'border-slate-300 bg-white'}`}>{option.text}</button>)}</div>{selected ? <Feedback correct={correct}><strong>{correct ? '적절한 선택입니다.' : '다시 생각해 보세요.'}</strong> {item.limit}</Feedback> : null}</fieldset> })}</div></div>
    <div className="mt-8 border-t border-slate-200 pt-7"><h3 className="text-xl font-black">확인 문제 {LESSON11_QUIZ.length}개</h3><p className="mt-2 text-sm text-slate-600">모든 문제를 정답으로 제출해야 STEP 7을 완료할 수 있습니다. 오답은 바로 고쳐 다시 풀 수 있습니다.</p><div className="mt-5 grid gap-5">{LESSON11_QUIZ.map((question) => { const answer = activity.quizAnswers[String(question.id)] ?? ''; const submitted = activity.quizSubmitted.includes(question.id); const correct = activity.quizCorrect.includes(question.id); return <fieldset key={question.id} className="rounded-2xl border border-slate-200 p-5"><legend className="px-2 font-black">문제 {question.id}. {question.question}</legend><ChoiceButtons showId options={question.options.map((text, index) => ({ id: String.fromCharCode(65 + index), text }))} value={answer} disabled={submitted} onChoose={(choice) => chooseQuiz(question.id, choice)} />{!submitted ? <Button className="mt-4" disabled={answer === ''} onClick={() => submitQuiz(question.id, question.answer)}>문제 {question.id} 제출</Button> : <Feedback correct={correct}><strong>{correct ? '정답입니다.' : '오답입니다.'}</strong> {question.explanation}{!correct ? <Button className="mt-3" variant="secondary" onClick={() => retryQuiz(question.id)}><RotateCcw size={17} /> 오답 다시 풀기</Button> : null}</Feedback>}</fieldset> })}</div></div>
